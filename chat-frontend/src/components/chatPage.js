...
function ChatPage() {
  ...
  const SaveNewMessages = () => {
    if (Newmessages != null && Newmessages.length > 0) {
      axios
        .post(baseUrl + "Chat/SaveMessages", Newmessages)
        .then((res) => {
          if (res.data == true) {
            setOldMessagesState([]);
            setMessagesState([]);
  ...
  const LoadChat = (item) => {
    ...
    //retreive msgs
    axios
      .post(baseUrl + "Chat/GetMessages", data)
      .then((res) => {
        if (res.data) {
          debugger;
  ...
  function sendMessageToServer() {
    ...
    var sendMsgData = {
      ...
      Messagebody:
        data.UserId == request1.FriendOneId
          ? request1.FriendOneName + " SAYS : " + data.message
          : data.UserId == request1.FriendTwoId
          ? request1.FriendTwoName + " SAYS : " + data.message
          : "",
    };

    axios
      .post(baseUrl + "Chat/SaveMessages", sendMsgData)
      .then((res) => {
        if (res.data == true) {
          console.log("Message Saved Successfully!");
        }
      ...
      try {
        connection.start().then((res) => {
          ...
          connection.on("ReceiveMessage", function (data) {
            ...
            if (
              (data.UserId == request1.FriendTwoId &&
                data.FriendId == request1.FriendOneId) ||
              (data.UserId == request1.FriendOneId &&
                data.FriendId == request1.FriendTwoId)
            ) {
              var li = document.createElement("li");
              debugger;
              li.textContent =
  ...
  function onAddAsFriendClick(item) {
    ...
    request1.FriendOneId = data1.FriendOneId;
    axios
      .post(baseUrl + "Friend/AddAsFriend", request1)
      .then((response) => {
                    ...
                    return (
                      <li key={i}>
                        <GreyColorButton
                          variant="contained"
                          color="primary"
                          onClick={() => onAddAsFriendClick(item)}
                          style={{ width: "150px", height: "40px" }}
                        >
